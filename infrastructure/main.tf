...
# HTTP access for the public web server in Virginia.
resource "aws_security_group" "virginia_web" {
  provider    = aws.virginia
  name        = "virginia-web"
  description = "Allow public HTTP traffic to the Virginia web server"
  vpc_id      = aws_vpc.virginia_cloud.id
  ...
  tags = {
    Name = "Virginia-Web-SG"
  }
}

# HTTP access for the public web server in Ohio.
resource "aws_security_group" "ohio_web" {
  provider    = aws.ohio
  name        = "ohio-web"
  description = "Allow public HTTP traffic to the Ohio web server"
  vpc_id      = aws_vpc.ohio_cloud.id
...
resource "aws_instance" "north_virginia_public" {
  ...
  vpc_security_group_ids      = [aws_security_group.virginia_web.id]
...
resource "aws_instance" "ohio_public" {
  ...
  subnet_id                   = aws_subnet.public_subnet_ohio.id
  associate_public_ip_address = true
  vpc_security_group_ids      = [aws_security_group.ohio_web.id]
  user_data                   = file("${path.module}/scripts/web.sh")
  tags = {
